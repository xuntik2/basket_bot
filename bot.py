...
ИСПРАВЛЕНИЯ:
- ✅ scheduled_health_refresh из handlers.py (не services.py)
- ✅ await для set_database_health (async в services.py)
- ✅ MessageHandler с NEW_MEMBERS + ChatMemberHandler
...
def schedule_professional_holidays(scheduler: AsyncIOScheduler, application: Application) -> None:
    ...
    for key, holiday in PROFESSIONAL_HOLIDAYS.items():
        ...
        scheduler.add_job(safe_execute, trigger=DateTrigger(run_date=run_datetime, timezone=MSK), id=f'holiday_{key}_{holiday_date.isoformat()}', args=[send_professional_holiday, application, holiday['name']], replace_existing=True)
...
async def main() -> None:
    ...
    await runtime_state.set_database_health(db_ok, db_message)  # ✅ await (async в services.py)
    if not db_ok:
