...
- ✅ scheduled_* функции импортируются из handlers (не services)
- ✅ Правильная сигнатура: args=[scheduled_health_refresh, application]
- ✅ Убран new_member_handler (не существует)
- ✅ await для set_database_health()
- ✅ flask_thread передаётся в shutdown_manager.setup()
- ✅ loop.add_signal_handler вместо signal.signal
"""
...
def setup_scheduler(application: Application, runtime_state: BotRuntimeState, rate_limiter: RateLimiter) -> AsyncIOScheduler:
    ...
    # ✅ FIX v24.5: Правильная сигнатура - только application
    scheduler.add_job(
...
async def main() -> None:
    ...
    # ✅ FIX v24.5: await для async метода
