...
- ✅ new_member_handler импортируется из handlers
- ✅ MessageHandler, filters импортированы
- ✅ Убран дубликат импорта CommandHandler
- ✅ Убран BotCommandScopeType (не используется)
- ✅ Правильная сигнатура scheduler args=[func, application]
"""
...
def schedule_professional_holidays(scheduler: AsyncIOScheduler, application: Application) -> None:
    ...
    for key, holiday in PROFESSIONAL_HOLIDAYS.items():
        ...
        scheduler.add_job(
            safe_execute, trigger=DateTrigger(run_date=run_datetime, timezone=MSK),
            id=f'holiday_{key}_{holiday_date.isoformat()}',
            args=[send_professional_holiday, application, holiday['name']], replace_existing=True,
        )
...
def setup_scheduler(application: Application, runtime_state: BotRuntimeState, rate_limiter: RateLimiter) -> AsyncIOScheduler:
    ...
    scheduler.add_job(
        safe_execute, trigger=CronTrigger(minute='*/1', timezone=MSK), id='db_health_refresh',
        args=[scheduled_health_refresh, application], replace_existing=True,
...
def create_flask_app(runtime_state: BotRuntimeState, metrics: BotMetrics) -> Flask:
    ...
    def health():
        ...
        return jsonify({'status': 'ok', 'timestamp': datetime.now(MSK).isoformat(), 'timezone': 'Europe/Moscow', 'uptime': metrics.get_uptime_str(), 'database': runtime_state.get_database_health(), 'metrics': metrics.snapshot()})
    @app.route('/metrics')
    def metrics_endpoint():
        if runtime_state.is_shutdown_requested():
            return jsonify({'status': 'shutting_down'}), 503
        return jsonify({'counters': metrics.snapshot(), 'uptime': metrics.get_uptime_str(), 'uptime_seconds': metrics.get_uptime().total_seconds(), 'database': runtime_state.get_database_health()})
    return app
...
async def setup_bot_commands(application: Application) -> None:
    ...
    if not config:
        return
    user_commands = [BotCommand('start', 'Начать работу'), BotCommand('setbirthday', 'Указать день рождения (ДД-ММ)'), BotCommand('mybirthday', 'Посмотреть свой день рождения')]
    admin_commands = user_commands + [BotCommand('help', 'Показать справку'), BotCommand('poll', 'Создать опрос в группе'), BotCommand('stats', 'Полная статистика (Excel)'), BotCommand('monthlystats', 'Статистика за месяц'), BotCommand('addbirthday', 'Добавить ДР пользователю')]
    try:
...
async def main() -> None:
    ...
    await runtime_state.set_database_health(db_ok, db_message)  # ✅ FIX: await!
    if not db_ok:
        logger.warning('Database health check failed: %s', db_message)
        await db_manager.init_tables()
    application = Application.builder().token(config.bot_token).build()
    application.bot_data.update({'config': config, 'db_manager': db_manager, 'rate_limiter': rate_limiter, 'metrics': metrics, 'runtime_state': runtime_state})
    ...
    shutdown_manager.setup(application, scheduler, flask_thread)  # ✅ FIX: передан flask_thread
    await application.initialize()
    await application.start()
    await verify_chat_member_setup(application)  # ✅ FIX: self-check
