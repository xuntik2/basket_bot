    ...
    try:
        ...
        if config.group_chat_id:
            ...
            for admin_id in config.admin_user_ids:
                scope_admin = BotCommandScopeChat(chat_id=admin_id)
                await application.bot.set_my_commands(admin_commands, scope=scope_admin)
...
async def main() -> None:
    ...
    # ✅ ВЫЗЫВАЕМ метод класса правильно: через экземпляр db_manager
    await db_manager.verify_tables()
