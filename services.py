...
class BotConfig:
    def __init__(self):
        ...
        # ✅ УЛУЧШЕНИЕ: строгая валидация — если не int, то None (не строка)
        try:
            self.group_chat_id: Optional[int] = int(group_chat_id_str) if group_chat_id_str else None
        except (ValueError, TypeError):
            self.group_chat_id = None  # ✅ Было: group_chat_id_str
...
def build_user_mention(user_id: Optional[int], full_name: str, username: str = "") -> str:
    ...
    if user_id:
        # ✅ Всегда делаем clickable mention, если есть user_id
...
async def safe_execute(job_func: Callable, application: Application, *args, **kwargs):
    try:
        ...
        if config and config.owner_id:
            error_key = f"job:{job_func.__name__}:{type(e).__name__}:{truncate_text_safe(str(e))[:120]}"
            await notify_owner(application.bot, error_msg, config, error_key=error_key, use_cooldown=True)
...
def rate_limit_check(func: Callable) -> Callable:
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if rate_limiter and user_id and not await rate_limiter.is_allowed(user_id):
            message = update.effective_message
            if message:
                await message.reply_text("⏳ Слишком много запросов. Подождите минуту.")
            return
        return await func(update, context, *args, **kwargs)
    return wrapper
...
class DatabaseManager:
    ...
    async def verify_tables(self) -> bool:  # ✅ ПЕРЕИМЕНОВАНО из init_tables
        """✅ УЛУЧШЕНИЕ: название отражает реальное поведение — только проверка"""
        try:
            await self._execute_with_retry(lambda: self.supabase.table('polls').select('*').limit(1).execute(), "verify tables")
            logger.info("Таблицы проверены и доступны")
            return True
        except Exception as e:
    ...
    async def cleanup_old_locks(self, days: int = CLEANUP_LOCKS_DAYS) -> int:
        """✅ УЛУЧШЕНИЕ: возвращает количество УДАЛЁННЫХ локов, а не оставшихся"""
        try:
            async with self._locks_lock:
                ...
                for poll_id in old_ids:
                    self._poll_locks.pop(poll_id, None)
                # ✅ Возвращаем количество удалённых, а не оставшихся
                return len(old_ids)
        except Exception as e:
    ...
    async def mark_daily_job_done(self, job_name: str, run_date: str) -> bool:
        """✅ УЛУЧШЕНИЕ: upsert вместо insert для защиты от дублей"""
        try:
            data = {'job_name': job_name, 'run_date': run_date, 'created_at': datetime.now(MSK).isoformat()}
            # ✅ Используем upsert с on_conflict для защиты от дублей
            await self._execute_with_retry(
                lambda: self.supabase.table('job_runs').upsert(data, on_conflict='job_name,run_date').execute(),
                "mark_daily_job_done"
            )
            return True
        except Exception as e:
    ...
    async def ensure_user_exists(self, user_id: int, username: str, full_name: str) -> bool:
        """✅ УЛУЧШЕНИЕ: обновляет username/full_name если пользователь уже есть"""
        try:
            ...
            if not existing.data:
                ...
                # ✅ Обновляем username/full_name если они изменились
                current = existing.data[0]
                if current.get('username') != username or current.get('full_name') != full_name:
                    await self._execute_with_retry(
                    ...
                    logger.debug(f"Обновлены данные пользователя {user_id}: {username}, {full_name}")
                else:
...
async def send_birthday_greetings(application: Application) -> Dict[str, int]:
    ...
    for person in birthdays:
        ...
        text = (f"🎂 <b>Сегодня день рождения у {mention}!</b>\n"
               f"Поздравляем! Желаем крепкого здоровья, энергии, удачи, отличного настроения и ярких побед — и в жизни, и на площадке! 🏀")
...
async def scheduled_cleanup_locks(application: Application):
    ...
    if db_manager:
        deleted_count = await db_manager.cleanup_old_locks()  # ✅ Теперь возвращает удалённые
        cache_count = await db_manager.cleanup_cache()
        job_runs_count = await db_manager.cleanup_old_job_runs()
        logger.info(f"Очистка завершена: locks_deleted={deleted_count}, cache_cleared={cache_count}, job_runs_deleted={job_runs_count}")
