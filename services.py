...
Версия 24.5 — Final Fixed
ИЗМЕНЕНИЯ В ВЕРСИИ 24.5:
- ✅ Убраны scheduled_* функции (перенесены в handlers.py)
- ✅ threading.Lock для cross-thread доступа (Flask + async)
- ✅ _OWNER_ERROR_CACHE_LOCK используется в _should_notify_owner
- ✅ activate_user() для повторного входа
- ✅ notify_owner с cooldown
- ✅ upsert для responses
"""
import os
import logging
import asyncio
...
_OWNER_ERROR_CACHE_LOCK = ThreadLock()  # ✅ threading.Lock для синхронной функции


def _should_notify_owner(error_key: str) -> bool:
    """Проверка cooldown для уведомлений владельцу"""
    now = datetime.now(MSK)
    with _OWNER_ERROR_CACHE_LOCK:  # ✅ FIX v24.5: Добавлена блокировка
...
# ==================== МЕТРИКИ ====================
class BotMetrics:
    """Потокобезопасные метрики с threading.Lock для cross-thread доступа"""
    def __init__(self):
