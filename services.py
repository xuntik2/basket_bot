...
- ✅ threading.Lock → ThreadLock для cross-thread доступа (Flask + async)
- ✅ set_database_health() теперь async + await
...
- ✅ upsert для save_response (вместо select+insert/update)
- ✅ Poll text с "завтра" + дата в скобках
...
# ==================== МЕТРИКИ ====================
class BotMetrics:
    """Потокобезопасные метрики с ThreadLock для cross-thread доступа"""
    def __init__(self):
...
async def refresh_database_health(state: BotRuntimeState, db_manager: 'DatabaseManager') -> None:
    """Однократное обновление health state"""
    try:
    ...
    *,
    ...
    try:
        ...
        if use_cooldown and not _should_notify_owner(key):
            logger.info("Уведомление владельцу подавлено cooldown: %s", key)
            return False
...
async def safe_execute(job_func: Callable, application: Application, *args, **kwargs):
    try:
        ...
        if config and config.owner_id:
            await notify_owner(
                application.bot, error_msg, config,
...
async def verify_chat_member_setup(application: Application) -> None:
    ...
    try:
        ...
        logger.info(
            "Проверка chat_member setup: bot_id=%s, chat_id=%s, status=%s",
            me.id, config.group_chat_id, getattr(member, "status", "unknown"),
        )
...
class DatabaseManager:
    ...
    async def poll_exists(self, poll_id: str) -> bool:
        try:
            result = await self._execute_with_retry(
                lambda: self.supabase.table('polls').select('poll_id').eq('poll_id', poll_id).limit(1).execute(),
                "poll_exists"
            )
    ...
    async def get_poll_training_date(self, poll_id: str) -> Optional[str]:
        try:
            result = await self._execute_with_retry(
                lambda: self.supabase.table('polls').select('training_date').eq('poll_id', poll_id).limit(1).execute(),
    ...
    async def save_response(self, poll_id: str, user_id: int, username: str,
                            ...
                            metrics: BotMetrics = None) -> bool:
        """✅ FIX v25.2: upsert вместо select+insert/update"""
        ...
        async with poll_lock:
            try:
                ...
                await self._execute_with_retry(
                    lambda: self.supabase.table('responses').upsert(data, on_conflict='poll_id,user_id').execute(),
                    "upsert response"
                )
    ...
    async def cleanup_old_job_runs(self, days: int = JOB_RUNS_CLEANUP_DAYS) -> int:
        try:
            ...
            result = await self._execute_with_retry(
                lambda: self.supabase.table('job_runs').delete().lt('run_date', cutoff_date.isoformat()).execute(),
    ...
    async def was_daily_job_done(self, job_name: str, run_date: str) -> bool:
        try:
            result = await self._execute_with_retry(
                lambda: self.supabase.table('job_runs').select('*').eq('job_name', job_name).eq('run_date', run_date).limit(1).execute(),
                "was_daily_job_done"
            )
            return bool(result.data)
        except Exception as e:
    ...
    async def activate_user(self, user_id: int, username: str, full_name: str) -> bool:
        """✅ FIX v25.2: Активация пользователя при повторном входе"""
        try:
            await self._execute_with_retry(
    ...
    async def get_monthly_stats(self, year: int, month: int, limit: int = 1000) -> pd.DataFrame:
        try:
            ...
            polls_result = await self._execute_with_retry(
                lambda: self.supabase.table('polls').select('*').gte('training_date', start_date).lt('training_date', end_date).limit(limit).execute(),
                "get_monthly_stats"
            )
    ...
    async def get_all_stats(self, limit: int = 5000) -> pd.DataFrame:
        try:
            ...
            polls_result = await self._execute_with_retry(
                lambda: self.supabase.table('polls').select('poll_id, training_date').in_('poll_id', poll_ids).execute(),
                "get polls for stats"
            )
    ...
    async def get_today_birthdays(self) -> List[Dict]:
        try:
            ...
            result = await self._execute_with_retry(
                lambda: self.supabase.table('birthdays').select('*').eq('birth_date', today_str).eq('is_active', True).execute(),
                "get_today_birthdays"
            )
    ...
    async def get_user_birthday(self, user_id: int) -> Optional[str]:
        try:
            result = await self._execute_with_retry(
                lambda: self.supabase.table('birthdays').select('*').eq('user_id', user_id).limit(1).execute(),
                "get_user_birthday"
            )
...
class PollManager:
    ...
    def create_poll_text(training_date: str) -> str:
        """✅ FIX v25.2: Добавлено слово 'завтра' + дата в скобках"""
...
async def chat_member_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if new_status in ['member', 'administrator'] and old_status not in ['member', 'administrator']:
            ...
            if skip:
                logger.info(f"Антидубль: пропускаем повторное приветствие chat_id={chat.id}, user_id={user.id}")
                return
            db_manager = context.application.bot_data.get('db_manager')
            if db_manager:
                await db_manager.activate_user(
                    user_id=user.id,
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE, user):
    """✅ ЭКСПОРТИРУЕТСЯ для использования в handlers.py"""
    try:
        full_name = user.full_name or user.first_name or "Коллега"
        welcome_text = (
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=f"ℹ️ {full_name}, чтобы я мог поздравлять тебя с днём рождения, открой личный чат со мной и отправь:\n/setbirthday ДД-ММ"
            )
...
async def scheduled_cleanup_locks(application: Application):
    """✅ ЭКСПОРТИРУЕТСЯ для использования в bot.py"""
    db_manager = application.bot_data.get('db_manager')
...
async def scheduled_health_refresh(application: Application) -> None:
    """✅ FIX v25.2: Однократное обновление health state (без while loop)"""
    db_manager = application.bot_data.get('db_manager')
