...
Версия 25.5 — Production Ready
ИСПРАВЛЕНИЯ:
- ✅ welcome_new_member экспортируется
- ✅ validate_birth_date_dd_mm экспортируется
- ✅ scheduled_health_refresh вместо periodic_health_refresh
- ✅ threading.Lock → ThreadLock для cross-thread
- ✅ activate_user() для повторного входа
- ✅ should_skip_welcome для антидубля
- ✅ notify_owner с cooldown
- ✅ upsert для save_response
- ✅ Poll text с "завтра" + дата
"""
import os
import logging
import asyncio
...
DB_MAX_RETRIES = 3
WELCOME_DEDUP_TTL_SECONDS = 90
OWNER_NOTIFY_COOLDOWN_SECONDS = 600

# ==================== КЭШ ДЛЯ УВЕДОМЛЕНИЙ ====================
_OWNER_ERROR_CACHE: Dict[str, datetime] = {}
...
def _should_notify_owner(error_key: str) -> bool:
    ...
    with _OWNER_ERROR_CACHE_LOCK:
        ...
        if last_sent and (now - last_sent).total_seconds() < OWNER_NOTIFY_COOLDOWN_SECONDS:
            return False
        _OWNER_ERROR_CACHE[error_key] = now
        return True

# ==================== КЭШ ДЛЯ ПРИВЕТСТВИЙ ====================
_recent_welcomes: Dict[str, float] = {}
...
async def should_skip_welcome(chat_id: Any, user_id: int, ttl_seconds: int = WELCOME_DEDUP_TTL_SECONDS) -> bool:
    ...
    async with _recent_welcomes_lock:
        expired_keys = [k for k, ts in _recent_welcomes.items() if now - ts > ttl_seconds]
...
class BotMetrics:
    def __init__(self):
        ...
        self._counters: Dict[str, int] = {'polls_created': 0, 'responses_saved': 0, 'errors_count': 0, 'birthdays_sent': 0, 'news_sent': 0, 'api_calls': 0}
...
async def refresh_database_health(state: BotRuntimeState, db_manager: 'DatabaseManager') -> None:
    try:
        ...
        await state.set_database_health(ok, message)
...
class BotConfig:
    def __init__(self):
        ...
        logger.info("Используется SUPABASE_URL без rewrite (supabase-py REST API)")
        admin_ids_str = os.environ.get('ADMIN_USER_IDS', '')
        self.admin_user_ids: List[int] = [int(x.strip()) for x in admin_ids_str.split(',') if x.strip()]
        ...
        try:
            ...
            logger.error(f"GROUP_CHAT_ID должен быть числом, получено: {group_chat_id_str}")
...
class RateLimiter:
    ...
    async def is_allowed(self, user_id: int) -> bool:
        async with self._global_lock:
            now = time.time()
            self.user_calls[user_id] = [t for t in self.user_calls[user_id] if now - t < self.period]
    ...
    async def cleanup_old_users(self, max_age_hours: int = 24) -> int:
        async with self._global_lock:
            ...
            old_users = [uid for uid, calls in self.user_calls.items() if not calls or max(calls) < cutoff]
...
def secure_compare(secret1: str, secret2: str) -> bool:
    return hmac.compare_digest(secret1.encode('utf-8') if isinstance(secret1, str) else secret1, secret2.encode('utf-8') if isinstance(secret2, str) else secret2)
...
async def notify_owner(bot: Bot, text: str, config: BotConfig, *, error_key: Optional[str] = None, use_cooldown: bool = True) -> bool:
    if not config.owner_id:
        return False
    try:
        key = error_key or f"notify:{truncate_text_safe(text)[:120]}"
        if use_cooldown and not _should_notify_owner(key):
            logger.info("Уведомление подавлено cooldown: %s", key)
            return False
        await bot.send_message(chat_id=config.owner_id, text=text)
        return True
    except Exception as e:
        logger.debug(f"Не удалось отправить уведомление: {e}")
        return False
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if config and config.owner_id:
        error_text = f"❌ Ошибка в боте: {type(error).__name__}: {truncate_text_safe(str(error))}"
...
async def safe_execute(job_func: Callable, application: Application, *args, **kwargs):
    try:
        ...
        if config and config.owner_id:
            await notify_owner(application.bot, error_msg, config, error_key=f"job:{job_func.__name__}:{type(e).__name__}:{truncate_text_safe(str(e))[:120]}", use_cooldown=True)
...
async def verify_chat_member_setup(application: Application) -> None:
    ...
    try:
        ...
        member = await application.bot.get_chat_member(config.group_chat_id, me.id)
        logger.info("Проверка chat_member setup: bot_id=%s, chat_id=%s, status=%s", me.id, config.group_chat_id, getattr(member, "status", "unknown"))
        if getattr(member, "status", None) not in ("administrator", "member"):
            logger.warning("Бот имеет неожиданный статус в группе: %s", getattr(member, "status", None))
    except Exception as e:
        logger.warning("Не удалось выполнить verify_chat_member_setup: %s", e)
...
class DatabaseManager:
    def __init__(self, supabase_client: Client):
        ...
        self._lock_created: Dict[str, datetime] = {}
    ...
    async def _execute_with_retry(self, func: Callable, description: str = "DB operation") -> Any:
        @retry(stop=stop_after_attempt(DB_MAX_RETRIES), wait=wait_exponential(multiplier=1, min=2, max=10), retry=retry_if_exception_type(Exception), reraise=True)
        def _execute():
            return func()
        try:
            return await asyncio.wait_for(asyncio.to_thread(_execute), timeout=DB_TIMEOUT_SECONDS)
    ...
    async def _get_poll_lock(self, poll_id: str) -> asyncio.Lock:
        async with self._locks_lock:
            if poll_id not in self._poll_locks:
                ...
                self._lock_created[poll_id] = datetime.now(MSK)
    ...
    async def _cleanup_locks_unsafe(self) -> None:
        ...
        old_ids = []
        for poll_id, created_at in list(self._lock_created.items()):
            if created_at < cutoff_date:
                old_ids.append(poll_id)
        for poll_id in old_ids:
            self._poll_locks.pop(poll_id, None)
            self._lock_created.pop(poll_id, None)
        if old_ids:
            logger.info(f"Очищено {len(old_ids)} старых locks")

    async def check_database_health(self) -> Tuple[bool, str]:
        try:
            await self._execute_with_retry(lambda: self.supabase.table('polls').select('*').limit(1).execute(), "health check")
    ...
    async def init_tables(self) -> bool:
        try:
            await self._execute_with_retry(lambda: self.supabase.table('polls').select('*').limit(1).execute(), "init tables")
    ...
    async def poll_exists(self, poll_id: str) -> bool:
        try:
            result = await self._execute_with_retry(lambda: self.supabase.table('polls').select('poll_id').eq('poll_id', poll_id).limit(1).execute(), "poll_exists")
    ...
    async def get_poll_training_date(self, poll_id: str) -> Optional[str]:
        try:
            result = await self._execute_with_retry(lambda: self.supabase.table('polls').select('training_date').eq('poll_id', poll_id).limit(1).execute(), "get_poll_training_date")
            if result.data:
    ...
    async def save_poll(self, poll_id: str, message_id: int, chat_id: int, training_date: str, created_at: datetime, metrics: BotMetrics = None) -> bool:
        try:
            data = {'poll_id': poll_id, 'message_id': message_id, 'chat_id': chat_id, 'training_date': training_date, 'created_at': created_at.isoformat()}
            await self._execute_with_retry(lambda: self.supabase.table('polls').upsert(data, on_conflict='poll_id').execute(), "save_poll")
    ...
    async def save_response(self, poll_id: str, user_id: int, username: str, full_name: str, response: str, metrics: BotMetrics = None) -> bool:
        ...
        async with poll_lock:
            try:
                data = {'poll_id': poll_id, 'user_id': user_id, 'username': username, 'full_name': full_name, 'response': response, 'created_at': datetime.now(MSK).isoformat(), 'updated_at': datetime.now(MSK).isoformat()}
                await self._execute_with_retry(lambda: self.supabase.table('responses').upsert(data, on_conflict='poll_id,user_id').execute(), "upsert response")
    ...
    async def get_poll_responses(self, poll_id: str, use_cache: bool = True) -> List[Dict]:
        ...
        try:
            result = await self._execute_with_retry(lambda: self.supabase.table('responses').select('*').eq('poll_id', poll_id).execute(), "get_poll_responses")
            data = result.data or []
            if use_cache:
                async with self._cache_lock:
    ...
    async def cleanup_old_locks(self, days: int = CLEANUP_LOCKS_DAYS) -> int:
        try:
            async with self._locks_lock:
                before = len(self._poll_locks)
                await self._cleanup_locks_unsafe()
                after = len(self._poll_locks)
                return before - after
        except Exception as e:
    ...
    async def cleanup_old_job_runs(self, days: int = JOB_RUNS_CLEANUP_DAYS) -> int:
        try:
            ...
            result = await self._execute_with_retry(lambda: self.supabase.table('job_runs').delete().lt('run_date', cutoff_date.isoformat()).execute(), "cleanup_old_job_runs")
    ...
    async def was_daily_job_done(self, job_name: str, run_date: str) -> bool:
        try:
            result = await self._execute_with_retry(lambda: self.supabase.table('job_runs').select('*').eq('job_name', job_name).eq('run_date', run_date).limit(1).execute(), "was_daily_job_done")
            return bool(result.data)
        except Exception as e:
    ...
    async def mark_daily_job_done(self, job_name: str, run_date: str) -> bool:
        try:
            await self._execute_with_retry(lambda: self.supabase.table('job_runs').insert({'job_name': job_name, 'run_date': run_date, 'created_at': datetime.now(MSK).isoformat()}).execute(), "mark_daily_job_done")
            return True
        except Exception as e:
    ...
    async def ensure_user_exists(self, user_id: int, username: str, full_name: str) -> bool:
        try:
            existing = await self._execute_with_retry(lambda: self.supabase.table('birthdays').select('*').eq('user_id', user_id).execute(), "ensure_user_exists")
            if not existing.data:
                data = {'user_id': user_id, 'username': username, 'full_name': full_name, 'birth_date': None, 'is_active': True, 'created_at': datetime.now(MSK).isoformat()}
                await self._execute_with_retry(lambda: self.supabase.table('birthdays').insert(data).execute(), "insert user")
    ...
    async def activate_user(self, user_id: int, username: str, full_name: str) -> bool:
        try:
            await self._execute_with_retry(lambda: self.supabase.table('birthdays').update({'is_active': True, 'username': username, 'full_name': full_name, 'updated_at': datetime.now(MSK).isoformat()}).eq('user_id', user_id).execute(), "activate_user")
    ...
    async def add_or_update_birthday(self, user_id: int, full_name: str, birth_date: str, username: str = '') -> bool:
        user_id = validate_user_id(user_id)
        birth_date = validate_birth_date_dd_mm(birth_date)
        try:
            existing = await self._execute_with_retry(lambda: self.supabase.table('birthdays').select('*').eq('user_id', user_id).execute(), "check birthday")
            data = {'user_id': user_id, 'full_name': full_name, 'birth_date': birth_date if birth_date else None, 'username': username, 'is_active': True, 'updated_at': datetime.now(MSK).isoformat()}
            if existing.data:
                await self._execute_with_retry(lambda: self.supabase.table('birthdays').update(data).eq('user_id', user_id).execute(), "update birthday")
            else:
                data['created_at'] = datetime.now(MSK).isoformat()
                await self._execute_with_retry(lambda: self.supabase.table('birthdays').insert(data).execute(), "insert birthday")
    ...
    async def get_monthly_stats(self, year: int, month: int, limit: int = 1000) -> pd.DataFrame:
        try:
            ...
            polls_result = await self._execute_with_retry(lambda: self.supabase.table('polls').select('*').gte('training_date', start_date).lt('training_date', end_date).limit(limit).execute(), "get_monthly_stats")
    ...
    async def get_all_stats(self, limit: int = 5000) -> pd.DataFrame:
        try:
            result = await self._execute_with_retry(lambda: self.supabase.table('responses').select('*').limit(limit).execute(), "get_all_stats")
            data = result.data or []
            if not data:
                return pd.DataFrame()
            poll_ids = list(set([r['poll_id'] for r in data]))
            polls_result = await self._execute_with_retry(lambda: self.supabase.table('polls').select('poll_id, training_date').in_('poll_id', poll_ids).execute(), "get polls for stats")
    ...
    async def get_today_birthdays(self) -> List[Dict]:
        try:
            ...
            result = await self._execute_with_retry(lambda: self.supabase.table('birthdays').select('*').eq('birth_date', today_str).eq('is_active', True).execute(), "get_today_birthdays")
    ...
    async def get_user_birthday(self, user_id: int) -> Optional[str]:
        try:
            result = await self._execute_with_retry(lambda: self.supabase.table('birthdays').select('*').eq('user_id', user_id).limit(1).execute(), "get_user_birthday")
    ...
    async def mark_user_inactive(self, user_id: int) -> bool:
        try:
            await self._execute_with_retry(lambda: self.supabase.table('birthdays').update({'is_active': False, 'updated_at': datetime.now(MSK).isoformat()}).eq('user_id', user_id).execute(), "mark_user_inactive")
            return True
        except Exception as e:
    ...
    async def check_news_sent(self, news_id: str) -> bool:
        try:
            result = await self._execute_with_retry(lambda: self.supabase.table('sports_news').select('*').eq('news_id', news_id).execute(), "check_news_sent")
            return bool(result.data)
        except Exception:
            return False
    ...
    async def mark_news_sent(self, news_id: str, title: str, metrics: BotMetrics = None) -> bool:
        try:
            data = {'news_id': news_id, 'title': title, 'sent_at': datetime.now(MSK).isoformat()}
            await self._execute_with_retry(lambda: self.supabase.table('sports_news').insert(data).execute(), "mark_news_sent")
            if metrics:
...
class PollManager:
    ...
    def create_poll_text(training_date: str) -> str:
        return (f"🏃‍♂️ <b>Коллеги, добрый день!</b>\n"
                f"Кто <b>завтра ({training_date})</b> идет на тренировку?\n"
    ...
    @staticmethod
    async def update_poll_message(bot: Bot, chat_id: int, message_id: int, poll_id: str, training_date: str, db_manager: DatabaseManager) -> bool:
        try:
            ...
            if later_users:
                text += f"\n{', '.join(later_users)}"
            await bot.edit_message_text(text=text, chat_id=chat_id, message_id=message_id, reply_markup=PollManager.create_keyboard(poll_id), parse_mode=ParseMode.HTML)
            return True
        except Exception as e:
...
async def chat_member_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if new_status in ['member', 'administrator'] and old_status not in ['member', 'administrator']:
            ...
            if db_manager:
                await db_manager.activate_user(user_id=user.id, username=user.username or '', full_name=user.full_name or user.first_name or "Коллега")
        ...
        logger.error(f"Ошибка в chat_member_handler: {e}", exc_info=True)

async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE, user):
    """✅ ЭКСПОРТИРУЕТСЯ для handlers.py"""
    try:
        ...
        private_text = (f"Привет, {full_name}! 👋\n"
                       f"Чтобы я мог автоматически поздравлять тебя с днём рождения, отправь мне в личку команду:\n"
                       f"/setbirthday ДД-ММ\n"
                       f"Пример:\n"
                       f"/setbirthday 15-03")
        try:
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f"ℹ️ {full_name}, чтобы я мог поздравлять тебя с днём рождения, открой личный чат со мной и отправь:\n/setbirthday ДД-ММ")
...
async def send_birthday_greetings(application: Application) -> Dict[str, int]:
    ...
    for person in birthdays:
        ...
        if in_chat is False:
            ...
            stats["skipped"] += 1
            continue
        text = f"🎂 <b>Сегодня день рождения у {mention}!</b>\nПоздравляем! Желаем крепкого здоровья, энергии, удачи, отличного настроения и ярких побед — и в жизни, и на площадке! 🏀"
        try:
            if os.path.exists(config.birthday_image_path):
                try:
                    with open(config.birthday_image_path, 'rb') as photo:
                        await application.bot.send_photo(chat_id=config.group_chat_id, photo=photo, caption=text, parse_mode=ParseMode.HTML)
                except Exception as file_error:
                    logger.error(f"Ошибка отправки фото: {file_error}")
                    await application.bot.send_message(chat_id=config.group_chat_id, text=text, parse_mode=ParseMode.HTML)
            else:
                await application.bot.send_message(chat_id=config.group_chat_id, text=text, parse_mode=ParseMode.HTML)
            ...
            stats["failed"] += 1
            if config.owner_id:
                await notify_owner(application.bot, f"❌ Ошибка поздравления {full_name}: {truncate_text_safe(str(e))}", config)
    return stats
...
async def run_daily_birthdays_with_guard(application: Application) -> bool:
    ...
    success = stats["attempted"] == 0 or (stats["sent"] > 0 and stats["failed"] == 0)
...
async def check_basketball_champions(application: Application):
    ...
    try:
        ...
        leagues = {'nba': ['нба', 'nba', 'чемпион нба', 'победитель нба'], 'vtb': ['втб', 'единая лига', 'чемпион единой лиги', 'победитель втб'], 'euroleague': ['евролига', 'чемпион евролиги', 'победитель евролиги']}
...
async def scheduled_health_refresh(application: Application) -> None:
    """✅ FIX v25.5: Однократное обновление (без while loop)"""
    db_manager = application.bot_data.get('db_manager')
    runtime_state = application.bot_data.get('runtime_state')
    if not db_manager or not runtime_state:
        return
    try:
        ok, message = await db_manager.check_database_health()
        await runtime_state.set_database_health(ok, message)
    except Exception as e:
