...
ИСПРАВЛЕНИЯ:
- ✅ scheduled_* функции (только здесь, не в services.py)
- ✅ new_member_handler с should_skip_welcome
- ✅ validate_birth_date_dd_mm в импортах
- ✅ rollback в poll_command
"""
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"👋 Привет, {user.first_name}!\nЯ бот для учета посещаемости тренировок.\nКаждый понедельник создаю опрос о посещении.\n/help - Помощь")
...
async def set_birthday_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await db_manager.add_or_update_birthday(user_id=user.id, full_name=user.full_name or user.first_name, birth_date=birth_date, username=user.username or '')
...
async def my_birthday_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not birth_date:
            await update.message.reply_text("📭 У вас пока не сохранена дата рождения.\nУкажите её так:\n/setbirthday ДД-ММ")
            return
...
async def poll_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if chat.type not in ("group", "supergroup"):
            await update.message.reply_text("❌ Команду /poll нужно запускать прямо в группе.\nВ личном чате опрос не создаётся.")
            return
        ...
        message = await context.bot.send_message(chat_id=target_chat_id, text=text, reply_markup=PollManager.create_keyboard(poll_id), parse_mode=ParseMode.HTML)
        
        saved = await db_manager.save_poll(poll_id=poll_id, message_id=message.message_id, chat_id=message.chat_id, training_date=training_date_iso, created_at=datetime.now(MSK), metrics=metrics)
        if not saved:
            logger.error(f"Не удалось сохранить ручной опрос {poll_id} в БД, удаляем сообщение")
            try:
                await context.bot.delete_message(chat_id=target_chat_id, message_id=message.message_id)
        ...
        config = context.application.bot_data.get('config')
        if config and config.owner_id:
            await notify_owner(context.bot, f"❌ Ошибка в /poll: {type(e).__name__}: {truncate_text_safe(str(e))}", config)
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            await context.bot.send_document(chat_id=user.id, document=output, filename=f"stats_{datetime.now(MSK).strftime('%Y%m%d')}.xlsx", caption="📊 Статистика")
        finally:
            output.close()
...
async def monthly_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        try:
            await context.bot.send_document(chat_id=user.id, document=output, filename=f"stats_{year}_{month:02d}.xlsx", caption=f"📊 Статистика за {month_name}")
        finally:
            output.close()
    except Exception as e:
...
async def poll_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        db_manager = context.application.bot_data.get('db_manager')
        metrics = context.application.bot_data.get('metrics')
        saved = await db_manager.save_response(poll_id=poll_id, user_id=user.id, username=user.username or '', full_name=user.full_name or user.first_name or '', response=response, metrics=metrics)
        if not saved:
        ...
        await PollManager.update_poll_message(bot=context.bot, chat_id=message.chat_id, message_id=message.message_id, poll_id=poll_id, training_date=training_date, db_manager=db_manager)
...
async def scheduled_poll(application: Application):
    ...
    if await db_manager.poll_exists(poll_id):
        ...
        return
    text = PollManager.create_poll_text(training_date)
    message = await application.bot.send_message(chat_id=config.group_chat_id, text=text, reply_markup=PollManager.create_keyboard(poll_id), parse_mode=ParseMode.HTML)
    saved = await db_manager.save_poll(poll_id=poll_id, message_id=message.message_id, chat_id=message.chat_id, training_date=training_date_iso, created_at=datetime.now(MSK), metrics=metrics)
    if not saved:
        logger.error(f"Не удалось сохранить опрос {poll_id} в БД, удаляем сообщение")
        try:
            await application.bot.delete_message(chat_id=config.group_chat_id, message_id=message.message_id)
        except Exception as delete_error:
...
async def scheduled_monthly_stats(application: Application):
    ...
    for admin_id in config.admin_user_ids:
        try:
            df = await db_manager.get_monthly_stats(year, month)
            if df.empty:
                await application.bot.send_message(chat_id=admin_id, text=f"📭 Статистика за {month_name} пуста")
                continue
            output = create_excel_from_dataframe(df, f"stats_{year}_{month:02d}")
            try:
                await application.bot.send_document(chat_id=admin_id, document=output, filename=f"stats_{year}_{month:02d}.xlsx", caption=f"📊 Автоматическая статистика за {month_name}")
            finally:
                output.close()
...
async def scheduled_health_refresh(application: Application):  # ✅ ТОЛЬКО ЗДЕСЬ
