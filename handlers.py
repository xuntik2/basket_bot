#!/usr/bin/env python3
"""
Обработчики команд бота + Scheduled Jobs
Версия 24.5 — Final Fixed
ИЗМЕНЕНИЯ В ВЕРСИИ 24.5:
- ✅ Добавлены scheduled_* функции (перенесены из services.py)
- ✅ Правильная сигнатура: async def func(application: Application)
...
# ==================== SCHEDULED JOBS (перенесены из services.py) ====================
...
async def scheduled_cleanup_locks(application: Application):
    """✅ FIX v24.5: Очистка старых локов и кэша"""
    db_manager = application.bot_data.get('db_manager')
...
async def scheduled_health_refresh(application: Application):
    """✅ FIX v24.5: Обновление статуса БД для Flask endpoint'а"""
    db_manager = application.bot_data.get('db_manager')
