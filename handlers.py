...
async def my_birthday_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not birth_date:
            ...
            return
        await message.reply_text(f"🎂 Ваша дата рождения: {birth_date}")
    except Exception as e:
...
async def poll_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        message = update.effective_message
        if message:
            await message.reply_text(f"❌ Ошибка создания опроса: {type(e).__name__}")
        config = context.application.bot_data.get('config')
        if config and config.owner_id:
            await notify_owner(
                context.bot,
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not message:
            return
        db_manager = context.application.bot_data.get('db_manager')
        await message.reply_text("📊 Формирую статистику...")
        df = await db_manager.get_all_stats()
        if df.empty:
            await message.reply_text("📭 Статистика пуста")
            return
...
async def monthly_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await message.reply_text(f"📊 Статистика за {month_name}...")
...
async def add_birthday_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        message = update.effective_message
        if message:
            await message.reply_text(f"❌ Ошибка: {ve}")
    except Exception as e:
